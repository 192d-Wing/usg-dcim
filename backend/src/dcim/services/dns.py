...
_HICKORY_FORWARD_ZONE_HEADER = "[[zones]]"
...
def _hickory_forward_zone(
    ...
    """Emit one Hickory Forward-zone TOML block. `upstreams` is a list
    of (host, port) tuples; the renderer normalizes the zone name to
    trailing-dot form and the upstream list to an inline-table array."""
    zone_label = zone if zone == "." else zone.rstrip(".") + "."
    ns_entries = ", ".join(_hickory_ns_entry(h, p) for h, p in upstreams)
    return [
        _HICKORY_FORWARD_ZONE_HEADER,
        f'zone = "{zone_label}"',
...
def _split_host_port(target: str, default_port: int) -> tuple[str, int]:
    ...
    operator upstream strings into (host, port). IPv6 literals get
    bracketed in the standard `[::1]:5353` form; bare addresses use
    the default port."""
...
def _hickory_ns_entry(host: str, port: int) -> str:
    """TOML inline-table for one Hickory upstream — Hickory wants
    `{ socket_addr = "<host>:<port>", protocol = "udp" }` per entry."""
    sock = f"[{host}]:{port}" if ":" in host else f"{host}:{port}"
    return f'{{ socket_addr = "{sock}", protocol = "udp" }}'
