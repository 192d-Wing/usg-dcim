...
Coverage 84.4% — the uncovered statements are in `setup()` (the
...
- **`quick-smoke`** — hand-rolled Corefile + zone + keys, useful
  for validating plugin changes in isolation.
- **`dcim-bundle-smoke`** — drives the DCIM Python renderer end-
  to-end (zone + key + Corefile all generated by
  `backend/src/dcim/services/dns.py`) and runs the custom CoreDNS
  image against the output. Closer to production behavior.

Each harness has its own README walking through the bring-up.
Both produce `dig +dnssec` output operators can verify visually.
